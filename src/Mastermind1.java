...
public class Mastermind1 extends JFrame implements ActionListener {
	...
	JPanel colouredPanel = new JPanel();	//creating Jpanels in the screen named colouredPanel, whitesPanel, blacksPanel and computerGuessPanel
	...
	static int whites (int [] one, int [] two)	//method that takes in two arrays and returns an int named one and two
	{
		...
		{ 
			...
			{ 
				if (i!=j && oneA[i]==twoA[j])	//the pegs in the same postion are not compared so to find if the pegs are in the wrong place(basically check if its the right colour but in the wrong place)
				{
					val++;	//increment the value of val(val here is the number of white pegs)
					oneA[i]=0-i-10;		//set the value of oneA[i] as 0-index-10
	...
	public Mastermind1(int h, int w, int c) {	//constructor that takes height, width and the color
		...
		for (int k = 0; k < width; k++)		//loop makes the computers guess
		{
			...
			computerGuess[k].setVisible(false);	//computer guess made invisible
	...
	public void actionPerformed(ActionEvent e) {	
		...
		{  
			...
			if (n == JOptionPane.NO_OPTION) //if n=NO_OPTION then program is closed
				...
				dispose();
				new Mastermind1(height,width,numColors);
	...
	public static void main(String[] args) {
		...
		new Mastermind1(10,4,5);	//new Mastermind obj created which gives 10 chances has 4 tiles across and 5 different colours
