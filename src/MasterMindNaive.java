...
public class MasterMindNaive extends JFrame implements ActionListener {
    ...
    int state[][];	//2d array of type int which changes the colour of the colouredpegs in the middle according to the int value it holds
    int CGstate[];	//
    int[] hiddenGuess;	//array of type int named hiddenGuess
    JButton guess = new JButton("Make Computer Guess");	//new button created with text "Guess" which when clicked is going to try and match with the computer's combination
    ...
    public MasterMindNaive(int h, int w, int c) {	//constructor that takes height, width and the color
        ...
        class CGuess implements ActionListener{
            public int x;
            public void actionPerformed(ActionEvent e) {
                CGstate[x]=(CGstate[x]+1)%numColors;	//changes the colour value when the button is clicked
                ((JButton)(e.getSource())).setBackground(choose(CGstate[x]));	//the background colour of the button is changed according to the clicks
            }
            
            public CGuess(int x){	//constructor that takes the int value which is the current button which is being clicked on
                this.x=x;
            }
        }
        ...
        for (int k = 0; k < width; k++)		//loop makes the computers guess
        {
            ...
            computerGuess[k].setVisible(true);	//computer guess made invisible
        ...
        for(int i=0;i<width;i++){		//only for computerGuessbuttons
            CGstate[i]=0;		//all the buttons start with red colour
            computerGuess[i].addActionListener(new CGuess(i));	//CGuess actionListener called on computer guess buttons
            set.addActionListener(new setCombi());	//setCombi actionlistener called on the set button
        }
        ...
        for (int i = 0; i < height; i++){ 	//loop that runs from 0 to the height
            for (int j = 0; j < width; j++)		//loop that runs from 0 to the width
            {
                ...
                state[i][j]=0;	//state is set to 0 so that to start off every color is red in the buttons
        ...
        topPanel.setLayout(new GridLayout(2,3));	//new gridlayout is set in the topPanel which has 1 row and 3 coulmns
        
        topPanel.add(none1,JButton.CENTER);
        none1.setVisible(false);
        topPanel.add(set,JButton.CENTER);
        topPanel.add(none2,JButton.CENTER);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        for(int i=0;i<width;i++){	//this loop make the buttons click 0011 combination which is the first combination according to the five guess algorithm
            ...
            for(int j=0;j<Character.getNumericValue((Pset.get(counter).charAt(index2)));j++){
                colouredPegs[numGuesses][i].doClick();
            }
            colouredPegs [numGuesses][i].setBackground(choose(Character.getNumericValue((Pset.get(counter).charAt(index2)))));
            index2++;
        }
        int whiteThings=whites(state[numGuesses],hiddenGuess);	//stores the number of white pegs to be displayed
        //System.out.println("num guesses "+java.util.Arrays.toString(state[numGuesses]));
        int blackThings=blacks(state[numGuesses],hiddenGuess);	//stores the number of black pegs to be displayed
        ...
        {
            for (int i=0;i<blackThings;i++)	//makes the 4 black pegs visible when the game is won
            ...
            if (n == JOptionPane.NO_OPTION) //if n=NO_OPTION then program is closed
                ...
                dispose();
                new MasterMindNaive(height,width,numColors);
            }
        }
    ...
    class setCombi implements ActionListener {
        @Override
        public void actionPerformed(ActionEvent e) {
            set.setEnabled(false);//once the set button is clicked, the button is disabled
            
            for(int i=0;i<width;i++){
                computerGuess[i].setEnabled(false);	//after setting the combnation the button will be unclickable
                //System.out.print((CGstate[i])+"    ");	//hiddenGuess  holds the int value of color
                hiddenGuess[i] = CGstate[i] ;	//This line set the hidden guess combination
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        new MasterMindNaive(10,4,5);	//new Mastermind obj created which gives 10 chances has 4 tiles across and 5 different colours
